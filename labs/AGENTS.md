...
5. Help select this project's `.venv` as the VS Code kernel: `.venv/Scripts/python.exe` on Windows or `.venv/bin/python` on macOS/Linux. Point the participant to the Setup cell in `hands_on.py`, or the same cell in `hands-on.ipynb`, and let them begin. Report the observed platform, versions and check result briefly; do not claim that testing one machine validates another platform.
...
- Some files contain worked references, including `pipeline.py`, `hands_on.py`, `hands-on.ipynb` and `hands-on.html`. Do not search or copy reference answers to solve a participant's exercise. You may inspect setup helpers narrowly for an installation error, without presenting the solution code you encounter.
