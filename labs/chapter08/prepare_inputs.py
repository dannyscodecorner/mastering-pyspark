...
def prepare(spark, destination):
    destination = Path(destination)
    spark.createDataFrame(SALES, SCHEMA).coalesce(1).write.mode("errorifexists").parquet(
        str(destination / "sales.parquet")
    )
    spark.createDataFrame(PRODUCTS, "product_id STRING, category STRING").coalesce(
        1
    ).write.mode("errorifexists").parquet(str(destination / "products.parquet"))
    for number, indices in enumerate(((0, 1), (2, 3, 5), (4, 6, 7)), 1):
        spark.createDataFrame([SALES[i] for i in indices], SCHEMA).coalesce(
            1
        ).write.mode("errorifexists").parquet(
            str(destination / "arrivals" / f"{number:02d}")
        )
