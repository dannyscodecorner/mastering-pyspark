...
class Workspace:
    ...
    def load(self, *names: str) -> tuple[Callable, ...]:
        ...
        if not self.answers.exists():
            raise FileNotFoundError(
                ...
                "Save and finish cells, or use the explicit catch-up step in docs/RECOVERY.md."
            )
        module = self._module()
        ...
        if missing:
            raise ValueError(
                ...
                "and its Save cell, or choose the matching recovery step in docs/RECOVERY.md."
            )
    ...
    def use_reference(self, through: str) -> None:
        ...
        definitions = self._definitions()
        reference_source = (LAB_ROOT / "lab_support/pipeline.py").read_text(encoding="utf-8")
        reference = {
