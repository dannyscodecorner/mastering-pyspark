...
The helper is written entirely in Incan, with no renderer dependencies or additional packages. It lives in `slides/src/helper.incn` for now and can move into Incan's standard library later. It follows Python-style dedenting for ordinary space/tab-indented blocks; it is not a claim of complete Python Unicode-whitespace compatibility.

Run its focused native tests from the repository root, with the build directories kept local to this checkout:

```sh
INCAN_HOME="$PWD/.build/incan-home" CARGO_TARGET_DIR="$PWD/.build/cargo" incan test slides/src/test_helper.incn --fail-on-empty
```
